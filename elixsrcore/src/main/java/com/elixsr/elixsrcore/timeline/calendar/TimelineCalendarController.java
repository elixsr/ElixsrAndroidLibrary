 ...
 */
public class TimelineCalendarController {

    private static final String TAG = "TimelineCalendarCont";
    ...
    private TimelineCalendarView.TimelineCalendarViewListener listener;
    private Map<String, List<Events>> eventsByMonthAndYearMap = new HashMap<>();
    private Map<String, boolean[]> eventPerDayByMonthAndYearMap = new HashMap<>();
    ...
    //to ensure no issues with above paint object
    private Paint alternateDayPaint = new Paint();
    private Paint currentSelectedDayPaint = new Paint();
    private Paint weekHeadingPaint = new Paint();
    ...
    private int weekHeadingTextColor;
    ...
    TimelineCalendarController(Paint dayPaint, OverScroller scroller, Rect textSizeRect,
                               AttributeSet attrs,
    ...
    private void loadAttributes(AttributeSet attrs, Context context) {
        if (attrs != null && context != null) {
            TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable
                    .TimelineCalendarView, 0, 0);
            try {
                currentDayBackgroundColor = typedArray.getColor(R.styleable
                        .TimelineCalendarView_timelineCalendarCurrentDayBackgroundColor,
                        currentDayBackgroundColor);
                calenderTextColor = typedArray.getColor(R.styleable.TimelineCalendarView_timelineCalendarTextColor, calenderTextColor);
                currentSelectedDayBackgroundColor = typedArray.getColor(R.styleable
                        .TimelineCalendarView_timelineCalendarCurrentSelectedDayBackgroundColor,
                        currentSelectedDayBackgroundColor);
                calenderBackgroundColor = typedArray.getColor(R.styleable
                        .TimelineCalendarView_timelineCalendarBackgroundColor, calenderBackgroundColor);
                multiEventIndicatorColor = typedArray.getColor(R.styleable
                        .TimelineCalendarView_timelineCalendarMultiEventIndicatorColor,
                        multiEventIndicatorColor);
                textSize = typedArray.getDimensionPixelSize(R.styleable
                        .TimelineCalendarView_timelineCalendarTextSize,
                ...
                targetHeight = typedArray.getDimensionPixelSize(R.styleable
                        .TimelineCalendarView_timelineCalendarTargetHeight,
                        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, targetHeight, context.getResources().getDisplayMetrics()));
                selectFirstDayOfMonthOnScroll = typedArray.getBoolean(R.styleable
                        .TimelineCalendarView_timelineCalendarSelectFirstDayOfMonthOnScroll, true);


                //if a day has no events - default to normal text color
                noEventDayTextColor = typedArray.getColor(R.styleable
                        .TimelineCalendarView_timelineCalendarNoEventDayColor, calenderTextColor);
                currentSelectedDayTextColor = typedArray.getColor(R.styleable
                        .TimelineCalendarView_timelineCalendarCurrentSelectedDayColor, calenderTextColor);
                weekHeadingTextColor = typedArray.getColor(R.styleable
                        .TimelineCalendarView_timelineCalendarDayHeadingColor, calenderTextColor);
            } finally {
                typedArray.recycle();
            }
        }
    ...
    private void init(Context context) {
        ...
        alternateDayPaint.setTextAlign(Paint.Align.CENTER);
        alternateDayPaint.setStyle(Paint.Style.STROKE);
        alternateDayPaint.setFlags(Paint.ANTI_ALIAS_FLAG);
        alternateDayPaint.setTypeface(Typeface.SANS_SERIF);
        alternateDayPaint.setTextSize(textSize);
        alternateDayPaint.setColor(noEventDayTextColor);
        alternateDayPaint.getTextBounds("31", 0, "31".length(), textSizeRect);
        alternateDayPaint.setTypeface(Typeface.DEFAULT_BOLD);

        currentSelectedDayPaint.setTextAlign(Paint.Align.CENTER);
        currentSelectedDayPaint.setStyle(Paint.Style.STROKE);
        currentSelectedDayPaint.setFlags(Paint.ANTI_ALIAS_FLAG);
        currentSelectedDayPaint.setTypeface(Typeface.SANS_SERIF);
        currentSelectedDayPaint.setTextSize(textSize);
        currentSelectedDayPaint.setColor(currentSelectedDayTextColor);
        currentSelectedDayPaint.getTextBounds("31", 0, "31".length(), textSizeRect);
        currentSelectedDayPaint.setTypeface(Typeface.DEFAULT_BOLD);

        weekHeadingPaint.setTextAlign(Paint.Align.CENTER);
        weekHeadingPaint.setStyle(Paint.Style.STROKE);
        weekHeadingPaint.setFlags(Paint.ANTI_ALIAS_FLAG);
        weekHeadingPaint.setTypeface(Typeface.SANS_SERIF);
        weekHeadingPaint.setTextSize(textSize);
        weekHeadingPaint.setColor(weekHeadingTextColor);
        weekHeadingPaint.getTextBounds("31", 0, "31".length(), textSizeRect);
        weekHeadingPaint.setTypeface(Typeface.DEFAULT_BOLD);
    ...
    void setListener(TimelineCalendarView.TimelineCalendarViewListener listener) {
        this.listener = listener;
    }

    void removeAllEvents() {
        eventsByMonthAndYearMap.clear();
        eventPerDayByMonthAndYearMap.clear();
    }
    ...
    void onSingleTapConfirmed(MotionEvent e) {
        ...
        if (dayOfMonth < calendarWithFirstDayOfMonth.getActualMaximum(Calendar.DAY_OF_MONTH)
            ...
            ///get the boolean map that tells us whether that date includes any events
            String monthKey = getKeyForCalendarEvent(calendarWithFirstDayOfMonth);
            boolean[] eventMap;
            if(eventPerDayByMonthAndYearMap.containsKey(monthKey)){
                eventMap = eventPerDayByMonthAndYearMap.get(monthKey);
            }else{
                return;
            }

            //we need to add one to the day of the month...
            //check if it contains an event, if so, then we will perform the selection logic, and hit up that lovely callback
            if(eventMap[dayOfMonth+1]) {
    ...
    void addEvent(Event event) {
        ...
        int tempCurrentDay = eventsCalendar.get(Calendar.DAY_OF_MONTH);

        boolean[] eventMap;

        if (eventPerDayByMonthAndYearMap.containsKey(key)) {
            eventMap = eventPerDayByMonthAndYearMap.get(key);
        }else{
            int monthDayCount = eventsCalendar.getActualMaximum(Calendar.DAY_OF_MONTH)+1;
            eventMap = new boolean[monthDayCount];
        }
        eventMap[tempCurrentDay] = true;

        eventPerDayByMonthAndYearMap.put(key, eventMap);
    }
    ...
    void removeEvent(Event event) {
        ...
        eventPerDayByMonthAndYearMap.remove(key);
    }
...
//        drawEvents(canvas, monthToDrawCalender, offset);
        ...
        String monthKey = getKeyForCalendarEvent(monthToDrawCalender);
        boolean[] eventMap;
        if(eventPerDayByMonthAndYearMap.containsKey(monthKey)){
            eventMap = eventPerDayByMonthAndYearMap.get(monthKey);
        }else{
            int monthDayCount = monthToDrawCalender.getActualMaximum(Calendar.DAY_OF_MONTH)+1;
            eventMap = new boolean[monthDayCount];
        }
...
//                    weekHeadingPaint.setColor(weekHeadingTextColor);
                    canvas.drawText(dayColumnNames[dayColumn], xPosition, paddingHeight, weekHeadingPaint);
                }
            } else {
                ...
                } else if (selectFirstDayOfMonthOnScroll && day == 1 && !isSameMonthAsCurrentCalendar && !isAnimatingWithExpose
                        ) {
                    drawCircle(canvas, xPosition, yPosition, currentSelectedDayBackgroundColor);
                }
...
//                    canvas.drawText(String.valueOf(day), xPosition, yPosition, dayPaint);
                    ...
                    if (currentCalender.get(Calendar.DAY_OF_MONTH) == day &&
                        ...
                        this.drawDayTextCurrentSelected(String.valueOf(day), xPosition, yPosition,
                                canvas);
                    }else {

                        if (eventMap[day]) {
                            this.drawDayTextWithEvent(String.valueOf(day), xPosition, yPosition,
                                    canvas);
                        } else {
                            this.drawDayTextWithoutEvent(String.valueOf(day), xPosition, yPosition,
                                    canvas);
                        }
                    }
                }
    ...
    private void drawDayTextWithEvent(String day, float xPosition, float yPosition, Canvas canvas){
        canvas.drawText(String.valueOf(day), xPosition, yPosition, dayPaint);
    }

    private void drawDayTextWithoutEvent(String day, float xPosition, float yPosition, Canvas
            canvas){
        canvas.drawText(String.valueOf(day), xPosition, yPosition, alternateDayPaint);
    }

    private void drawDayTextCurrentSelected(String day, float xPosition, float yPosition, Canvas
            canvas){
        canvas.drawText(String.valueOf(day), xPosition, yPosition, currentSelectedDayPaint);
    }
